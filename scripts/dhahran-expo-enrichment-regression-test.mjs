...
assert.ok(
  ...
  `build must carry Dhahran Expo program outlines into dist/events.json: ${distEnriched.length} in dist, ${enrichedCatalog.length} enriched in the catalog, ${excusedEnriched.length} recorded as dropped by the build (${excusedEnriched.map((event) => `${event.id}:${buildExclusions.get(String(event.id).normalize('NFC'))?.reason}`).join(', ') || 'none'})`
);
