...
assert.equal(owner.search_visibility_baseline?.site, 'https://eventme.live/');
assert.ok(Array.isArray(owner.search_visibility_baseline?.queries));
assert.match(ownerHtml, /<meta name="robots" content="noindex,nofollow"/);
assert.match(ownerHtml, /خط أساس ظهور Google/);
assert.match(ownerHtml, /فعاليات السعودية/);
