  ...
  return page.evaluate(() => {
    ...
    return {
      ...
      homeHeader: (() => {
        ...
        const language = document.querySelector('.site-head .language-switch');
        ...
        return header ? {
          ...
          languageWidth: languageRect?.width || 0,
          languageHeight: languageRect?.height || 0,
          languageShortLabel: language?.getAttribute('data-short-label') || '',
...
try {
  ...
  for (const width of widths) {
    ...
    assert.equal(home.homeHeader?.ctaDisplay, 'none', `home header must move the Now action into the live board at ${width}px`);
    assert.equal(home.homeHeader?.languageShortLabel, 'EN', `Arabic home must expose a compact English switch at ${width}px`);
    assert.ok(home.homeHeader?.languageWidth >= 44 && home.homeHeader?.languageWidth <= 48, `home language switch must stay compact at ${width}px`);
    assert.ok(home.homeHeader?.languageHeight >= 44, `home language switch must remain touch friendly at ${width}px`);
