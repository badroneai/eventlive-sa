...
if (sourceOps.status !== 'PASS') findings.push({ area: 'reliability', issue: 'source harvest OS status is not PASS' });
...
if (status !== 'PASS') {
  console.error(`OPS_READINESS_FAIL findings=${findings.length}`);
  process.exit(1);
}

console.log(`OPS_READINESS_OK checks=${loadResults.length} max_latency=${maxLatency}ms`);
